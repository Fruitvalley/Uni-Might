                ...
                <form action="" id="manage-model">
                    ...
                    <div class=" row form-group">
                        <div class="col-md-5">
                            ...
                            <input type="file" class="form-control" name="cover" onchange="displayImg2(this,$(this))">
                        </div>

                        <div class="col-md-5">
                            <img src="<?php echo isset($cover) ? 'assets/uploads/models/'.$cover :'' ?>" alt="" id="cover-field">
                        </div>
                    </div>
                        ...
                        <label for="chooseFile" id="choose"><strong>Choose File</strong></label>
                              <div id="drop">
                                <?php 
                                    $images = array();
                                    if(isset($id)){
                                        $fpath = 'assets/uploads/models/'.$id;
                                        if(is_dir($fpath))
...
<script>
    ...
    $('#manage-model').submit(function(e){
        e.preventDefault()
        start_load()
        $('#msg').html('')
        $.ajax({
            ...
            success:function(resp){
                if(resp==1){
                    ...
                    setTimeout(function(){
                        location.href = "index.php?page=models"
                    },1500)
  ...
  var drop;
  addmodelHandler(window, 'load', function() {
    ...
    addmodelHandler(drop, 'dragover', cancel);
    addmodelHandler(drop, 'dragenter', cancel);

    addmodelHandler(drop, 'drop', function(e) {
      e = e || window.model; // get window.model if e argument missing (in IE)   
      if (e.preventDefault) {
        e.preventDefault();
      ...
      for (var i = 0; i < files.length; i++) {
        ...
        //attach model handlers here...

        reader.readAsDataURL(file);
        addmodelHandler(reader, 'loadend', function(e, file) {
          var bin = this.result;
        ...
        }.bindTomodelHandler(file));
      }
      return false;

    });

    Function.prototype.bindTomodelHandler = function bindTomodelHandler() {
      var handler = this;
...
function addmodelHandler(obj, evt, handler) {
  if (obj.addmodelListener) {
    // W3C method
    obj.addmodelListener(evt, handler, false);
  } else if (obj.attachmodel) {
    // IE method.
    obj.attachmodel('on' + evt, handler);
  } else {
    // Old school method.
    obj['on' + evt] = handler;
  }
...
function displayImg2(input,_this) {
    if (input.files && input.files[0]) {
        var reader = new FileReader();
        reader.onload = function (e) {
            $('#cover-field').attr('src', e.target.result);
        }
